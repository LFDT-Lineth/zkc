...
func AlignTrace[F field.Element[F], M register.Map](schema []M, tr trace.Trace[F], expanding bool,
	...
	var (
		...
		modules  = make([]trace.ArrayModule[F], tr.Width())
...
func alignColumns[F field.Element[F]](mapping register.Map, module trace.Module[F], expanding bool,
	...
	for i := range width {
		...
		ncols[i] = trace.NewArrayColumn(ith.Name(), nil, zero)
		colmap[ith.Name()] = i
	}
	...
	for i := range module.Width() {
		...
		if !ok {
			...
			seen[cid] = true
			ncols[cid] = trace.NewArrayColumn(col.Name(), col.Data().Clone(), col.Padding())
			// Update height
			if isEmpty && data != nil {
				height = data.Len()
				isEmpty = false
	...
	//
	return trace.NewArrayModule(module.Name(), 0, ncols), errs
}
