...
func MarshalBinary[W util.BigInter](tr Trace[W]) ([]byte, error) {
	...
	for mid := uint(0); mid < tr.Width(); mid++ {
		if err := marshalModule(&buffer, tr.Module(mid)); err != nil {
			return nil, err
		}
	}
	//
...
func (p *Array[T]) UnmarshalBinary(data []byte) error {
	...
	//
	modules, err := unmarshalModules[T](buffer)
	if err != nil {
		return err
...
func marshalModule[W util.BigInter](buffer *bytes.Buffer, module Module[W]) error {
	...
	marshalLimbs(buffer, module.Limbs().Collect())
	writeUvarint(buffer, module.Height())
	writeUvarint(buffer, module.Width())
	//
...
func unmarshalModules[T any](buffer *bytes.Buffer) ([]ArrayModule[T], error) {
	...
	//
	return readSlice(buffer, unmarshalModule[T])
}

func unmarshalModule[T any](buffer *bytes.Buffer) (ArrayModule[T], error) {
	r := reader{buf: buffer}
	//
	name := r.str()
	registers := readWith(&r, readRegisters)
	moduleLimbs := readWith(&r, readLimbs)
	height := r.uvarint()
	width := r.uvarint()
	//
	if r.err != nil {
		return ArrayModule[T]{}, r.err
	} else if uint(len(moduleLimbs)) != width {
		return ArrayModule[T]{}, fmt.Errorf("invalid module limb count (have %d, expected %d)", len(moduleLimbs), width)
	}
	//
	if err := validateRegisterLimbs(registers, moduleLimbs); err != nil {
		return ArrayModule[T]{}, err
	}
	//
	var err error
	//
	...
	//
	return NewArrayModule(name, registers, rows...), nil
}
...
func readRegister(buffer *bytes.Buffer) (Register, error) {
	...
	limbs := readWith(&r, readLimbs)
	//
	if r.err != nil {
		return nil, r.err
	}
	//
	reg := register{name, bitwidth, limbs}
...
func readLimbs(buffer *bytes.Buffer) ([]limb, error) {
	return readSlice(buffer, readLimb)
}

func readLimb(buffer *bytes.Buffer) (limb, error) {
	r := reader{buf: buffer}
	//
	lid := r.uvarint()
	rid := r.uvarint()
	...
	//
	return newLimb(lid, rid, bitwidth), nil
}
...
func validateRegister(reg register) error {
	...
	//
	if len(reg.limbs) == 0 {
		return fmt.Errorf("register %q must have at least one limb", reg.Name())
	}
	//
	for _, limb := range reg.limbs {
		bitwidth := limb.Bitwidth()
		//
		if bitwidth.IsEmpty() {
			nativeLimbs++
		} else if totalSize+bitwidth.Unwrap() < totalSize {
			return fmt.Errorf("register %q bitwidth overflow", reg.Name())
		} else {
			totalSize += bitwidth.Unwrap()
		}
	}
	//
	if nativeLimbs > 1 {
		return fmt.Errorf("register %q has multiple limbs without bitwidths", reg.Name())
	} else if nativeLimbs == 1 && reg.Bitwidth().HasValue() {
		return fmt.Errorf("register %q has native limb but fixed bitwidth", reg.Name())
	} else if nativeLimbs == 0 && !equalOptionUint(reg.Bitwidth(), util.Some(totalSize)) {
		return fmt.Errorf("register %q bitwidth mismatch", reg.Name())
	}
	//
	return nil
}

func validateRegisterLimbs(registers []Register, moduleLimbs []limb) error {
	var flattened []Limb
	//
	for _, reg := range registers {
		flattened = append(flattened, reg.Limbs().Collect()...)
	}
	//
	if len(flattened) != len(moduleLimbs) {
		return fmt.Errorf("invalid flattened limb count (have %d, expected %d)", len(flattened), len(moduleLimbs))
	}
	//
	for i, left := range flattened {
		right := moduleLimbs[i]
		//
		if left.LimbId() != right.LimbId() ||
			left.RegisterId() != right.RegisterId() ||
			!equalOptionUint(left.Bitwidth(), right.Bitwidth()) {
			return fmt.Errorf("inconsistent limb descriptor at index %d", i)
		}
	}
	//
	return nil
...
func equalOptionUint(lhs util.Option[uint], rhs util.Option[uint]) bool {
...
func marshalRegisters(buffer *bytes.Buffer, registers []Register) {
	writeSlice(buffer, registers, func(buffer *bytes.Buffer, reg Register) {
		...
		marshalLimbs(buffer, reg.Limbs().Collect())
	})
}

func marshalLimbs(buffer *bytes.Buffer, limbs []Limb) {
	writeSlice(buffer, limbs, func(buffer *bytes.Buffer, limb Limb) {
		writeUvarint(buffer, limb.LimbId())
		writeUvarint(buffer, limb.RegisterId())
		writeOptionUint(buffer, limb.Bitwidth())
	})
}
