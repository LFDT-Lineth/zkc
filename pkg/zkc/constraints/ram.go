...
//
// TODO: source this per-memory once the stamp-width syntax lands (issue #1807,
// Step 5) rather than sharing a global default in two packages.
...
// translateReadWriteMemory.  Both the module translation (which creates the
// columns) and the caller->RAM lookup (which references them by id) derive the
// layout from this helper so the two stay in lock-step.
//
...
type ramLayout struct {
	...
	// timestamp / address additions.  Each has one fewer entry than the value it
	// carries (the most significant limb produces no carry), indexed by
	// significance (index 0 == carry out of the least significant limb).
	tsCarry   []register.Id
...
func translateReadWriteMemory[W vm.Word[W], F field.Element[F]](
	...
	// (EXEC == FINL == 0 there); memory modules are never native.
	mod = mod.Init(name, true, true, false, false, false, 0)
