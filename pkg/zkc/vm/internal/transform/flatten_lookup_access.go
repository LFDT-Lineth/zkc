...
func flattenLookupAccess[W word.Word[W]](code Bytecode[W], snapshot []bool,
	...
	case *bytecode.ReadWrite[W]:
		...
		// (possibly snapshotted) operands back into their fields.  Stamp is
		// present after timestamp threading and must be preserved.
		var (
			naddr   = len(c.Address)
			address = uses[:naddr]
			data    = c.Data
		...
		//
		if c.Write {
			data = rest[:len(c.Data)]
			rest = rest[len(c.Data):]
		}
		//
