...
func fieldTestProgram[W vm.Word[W]]() vm.Program[W] {
	...
	return vm.NewBytecodeProgram(
		...
		vm.NewBytecodeMemory[W]("data", vm.PUBLIC_READ_ONLY_MEMORY, util.None[uint](), memRegs()),
		vm.NewBytecodeMemory[W]("result", vm.PUBLIC_WRITE_ONCE_MEMORY, util.None[uint](), memRegs()),
