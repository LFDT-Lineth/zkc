...
type Memory[W word.Word[W]] interface {
	...
	Contents() []W
	// Accesses returns the chronological log of reads / writes performed against
	// this memory, or nil if it does not record accesses (only read-write memory
	// records, and only when a recording log is installed for trace generation).
	// The trace observer consumes this to materialise the RAM trace.
