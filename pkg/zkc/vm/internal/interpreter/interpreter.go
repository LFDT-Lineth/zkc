...
func (p *Interpreter[W]) executeAdd_nm(pc uint32, codes []uint32, stack []W) (uint32, error) {
	...
	for sources.HasNext() {
		...
		//
		if overflow {
			return pc, failure("arithmetic overflow")
		}
	}
	//
...
func (p *Interpreter[W]) executeFail(pc uint32, codes []uint32, frame []W) error {
	...
	//
	if len(chunks) == 0 {
		return failure("machine panic")
	}
	//
	return failure("%s", p.formatChunks(chunks, sources, frame))
}
...
func executeCheckCast[W word.Word[W]](pc uint32, codes []uint32, stack []W) (uint32, error) {
	...
	if !value.FitsWithin(uint(bitwidth)) {
		return pc, failure("bit overflow (0x%s not u%d)", value.Text(16), bitwidth)
	}
	//
	return pc + n, nil
}
...
func executeDiv[W word.Word[W]](pc uint32, codes []uint32, stack []W) (uint32, error) {
	...
	if stack[divisor].Cmp64(0) == 0 {
		return pc, failure("division by zero")
	}
	//
...
func (p *Interpreter[W]) executeDivHint(pc, n uint32, targets, sources encoding.Operands,
	...
	//
	if w.Sign() < 0 {
		return pc, failure("arithmetic underflow")
	}
...
func storeIntrinsicResult[W word.Word[W]](module descriptor.Module[W], iter *encoding.Operands,
	...
	//
	if acc.Sign() != 0 {
		return failure("bit overflow (0x%s not u%d)", value.Text(16), total)
	}
	//
	return nil
}
...
func storeAcross[W word.Word[W]](pc uint32, module descriptor.Module[W], targets encoding.Operands, oval W,
	...
	//
	if value.Cmp64(0) != 0 {
		return failure("bit overflow (0x%s not u%d, pc=0x%x)", oval.Text(16), bitwidth, pc)
	}
	//
	return nil
}
