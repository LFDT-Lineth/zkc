...
func (p *Compiler) buildMemory(program []Declaration, c *decl.ResolvedMemory,
	...
	case decl.PRIVATE_READ_ONLY_MEMORY:
		return vm.NewBytecodeMemory(c.Name(), vm.PRIVATE_READ_ONLY_MEMORY, 0, regs), nil
	case decl.PUBLIC_READ_ONLY_MEMORY:
		return vm.NewBytecodeMemory(c.Name(), vm.PUBLIC_READ_ONLY_MEMORY, 0, regs), nil
	case decl.PRIVATE_WRITE_ONCE_MEMORY:
		return vm.NewBytecodeMemory(c.Name(), vm.PRIVATE_WRITE_ONCE_MEMORY, 0, regs), nil
	case decl.PUBLIC_WRITE_ONCE_MEMORY:
		return vm.NewBytecodeMemory(c.Name(), vm.PUBLIC_WRITE_ONCE_MEMORY, 0, regs), nil
	case decl.PRIVATE_STATIC_MEMORY:
		...
		//
		return vm.NewBytecodeMemory(c.Name(), vm.PRIVATE_STATIC_MEMORY, 0, regs, words...), errs
	case decl.PUBLIC_STATIC_MEMORY:
		// Compile the static initialiser
		words, errs := p.compileStaticInitialisers(program, p.env, p.srcmaps, c.Contents...)
		//
		return vm.NewBytecodeMemory(c.Name(), vm.PUBLIC_STATIC_MEMORY, 0, regs, words...), errs
	case decl.RANDOM_ACCESS_MEMORY:
		...
		if slices.Contains(c.Annotations(), "paged") {
			return vm.NewBytecodeMemory(c.Name(), vm.PAGED_READWRITE_MEMORY, width, regs), nil
		}
		//
		return vm.NewBytecodeMemory(c.Name(), vm.READWRITE_MEMORY, width, regs), nil
	default:
